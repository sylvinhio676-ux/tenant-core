...
voir [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) — ce document-ci ne fait que
du "comment faire".
...
func main() {
	// 1. Store: source de vérité pour les données tenant. En production,
	// remplacez ceci par votre propre implémentation de l'interface
	// tenant.Store (Postgres, etc.) — tout le reste ci-dessous reste identique.
	...
	// 2. Resolver: identifie le tenant depuis le sous-domaine de la requête.
	// "acme.localhost" -> TenantID("acme")
	subResolver := resolver.NewSubdomainResolver("localhost")

	// 3. Manager: assemble Resolver + Store. Panique au démarrage si l'un
	// des deux manque — une erreur de configuration doit échouer
	// immédiatement, pas au moment d'une requête.
	manager := tenant.New(
		tenant.WithResolver(subResolver),
		tenant.WithStore(memStore),
	)

	// 4. Votre handler applicatif — n'a aucune connaissance de tenant-core,
	// il relit simplement le tenant depuis le contexte.
	...
	// 5. Middleware: résout le tenant, l'injecte dans le contexte de la
	// requête, et rejette avec 404 (avant que "next" ne soit jamais appelé)
	// en cas d'échec.
...
curl -H "Host: inconnu.localhost" http://localhost:8080/whoami
# 404 tenant not found — le tenant "inconnu" n'existe pas dans le Store
```
...
manager := tenant.New(
	tenant.WithResolver(subResolver),
	tenant.WithStore(cachedStore), // avant : memStore
)
```
...
```go
// LimitFunc décide du quota requêtes/seconde par tenant. Ici un flat
// 5 req/s pour tout le monde — en pratique, dérivez-le du plan du tenant
// (un champ que vous ajouteriez à vos propres métadonnées de Tenant, ou
// un lookup indexé par t.ID).
...
func main() {
	...
	// eventbus.NewMemoryEventBus() est mono-instance — suffisant pour
	// démarrer, mais chaque instance de votre serveur garderait sa propre
	// vue de qui est banni. Pour une propagation multi-instance, voir
	// eventbus/redis.
...
adminHandler := admin.NewHTTPHandler(
	adminService,
	admin.WithAuthenticator(monAuthenticator), // implémente admin.Authenticator
)
```
